...
class MakeDeterministicV2:
    def __init__(self,model):
        self.model=model
    def get_action(self,state):
        return self.model.get_action(state)[0]
    def update_history(self,a,r):
        pass
    def reset(self):
        pass
...
weight_path=f'{weight_path_dir}/policy_0.pth'
policy = TanhGaussianPolicy(
        obs_dim=state_dim,
        action_dim=act_dim,
        hidden_sizes=[M, M, M], )
policy.load_state_dict(torch.load(weight_path))
model = MakeDeterministic(policy)
model = MakeDeterministicV2(model)
...
model=RetrieverRLV2(retriever=retriever,model=model,lamb=0.5,solo=True,kwargs=kwargs)
...
data=pd.read_csv('cql_solo_exp.csv')
data.loc[len(data)]={
    'env':args.env,
    'mean_return':np.array(r_list).mean()
}
data.to_csv('cql_solo_exp.csv',index=False)